...
const protect = (req, res, next) => {
  try {
    ...
    req.token = token; // Store token for potential service calls
...
const protectWithServiceVerification = async (req, res, next) => {
  try {
    const token = req.header("Authorization")?.replace("Bearer ", "");
...
const serviceAuth = (req, res, next) => {
  try {
    ...
    // Verify service token using internal secret
...
module.exports = {
  protect,
  protectWithServiceVerification,
  enrichUserProfile,
  adminOnly,
  patientOnly,
  doctorOnly,
